...
;(function () {
    ...
    angular.module('demo', [
               ...
               'https://fast.wistia.net/embed/iframe/**']);
                ...
                if (isOnGitHub) {
                    ...
                    angular.extend(fileUploadProvider.defaults, {
                        ...
                        maxFileSize: 999000,
                        acceptFileTypes: /(\.|\/)(gif|jpe?g|png|3gp|mov|MOV)$/i
                    });
                }
            }
        ])
        ...
        .controller('DemoFileUploadController', [
            '$scope', '$http', '$filter', '$window','$sce',
            function ($scope, $http,$sce) {
                $scope.options = {
                    url: url
                };
                $scope.trustSrc = function(src,$sce) {
                    return $sce.trustAsResourceUrl("https://fast.wistia.net/embed/iframe/"+src);
                };
                if (!isOnGitHub) {
                    $scope.loadingFiles = true;
                    $http.post(url)
                        .success(function (data, status, headers, config) {
                            $scope.ResponseDetails = "Data: " + data.response.result +
                                "<br />status: " + status +
                                "<br />headers: " + (header) +
                                "<br />video Url: " + (config);
